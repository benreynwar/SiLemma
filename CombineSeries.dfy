module CombineSeries {
  ...
  datatype SeriesCombiner = SeriesCombiner(
    mf_a: MapFunction,
    mf_b: MapFunction
  ) {
    ...
    {
      seq(|mf_b.inputs|, (index: nat) requires index < |mf_b.inputs| => (true, index))
    }

    function AOBO2ABO(): (aobo2abo: seq<(bool, nat)>)
    {
      seq(|mf_b.outputs|, (index: nat) requires index < |mf_b.outputs| => (true, index))
    }
    ...
    function sf(si: SI): (so: SO)
      requires Valid()
      requires SIValid(si, mf_a.inputs, mf_a.state + mf_b.state)
      ensures SOValid(so, mf_b.outputs, mf_a.state + mf_b.state)
    {
      ...
      assert SOValid(so, mf_b.outputs, mf_a.state + mf_b.state);
      so
    }
    ...
    {
      reveal MapFunction.Valid();
      var r := MapFunction(mf_a.inputs, mf_b.outputs, mf_a.state + mf_b.state, sf);
      var inputs := r.inputs;
      var state := r.state;
      ...
      assert Seq.ToSet(mf_a.inputs) !! Seq.ToSet(StateONPsSeq(mf_a.state + mf_b.state)) by {
      ...
      assert Seq.ToSet(mf_b.outputs) !! Seq.ToSet(StateINPsSeq(mf_a.state + mf_b.state)) by {
    ...
    lemma ABI2AICorrect()
      ensures
        ConnectionX(mf_a.inputs, mf_a.inputs, ABI2AI()).Valid()
    {
    ...
    lemma ABIAO2BICorrect()
      requires Valid()
      ensures
        ConnectionXY(mf_a.inputs, mf_a.outputs, mf_b.inputs, true, false, ABIAO2BI()).Valid()
    {
      reveal Seq.HasNoDuplicates();
      ...
      assert Seq.ToSet(out) !! Seq.ToSet(in1);
      forall index_out: nat, index1: nat | index_out < |out| && index1 < |in1| && in1out_direct
        ensures out[index_out] != in1[index1]
      {
        if out[index_out] == in1[index1] {
          reveal Seq.ToSet();
          assert out[index_out] in Seq.ToSet(out);
    ...
    lemma {:vcs_split_on_every_assert} AOBO2ABOCorrect()
      requires Valid()
      ensures
        ConnectionXY(mf_a.outputs, mf_b.outputs, mf_b.outputs, true, true, AOBO2ABO()).Valid()
    {
      reveal Seq.HasNoDuplicates();
      reveal ConnectionXY<NP>.Valid();
      var out := mf_b.outputs;
      var conn := AOBO2ABO();
      ...
      ensures ConnImpl().Valid()
    {
        ...
        ensures conn.SeqEvaluateSeparately(si) == conn.mf_ab.sf(si)
      {
        ...
        // By mf_ab
        var si2_a := SI(si.inputs, si.state[..|mf_a.state|]);
        var so2_a := mf_a.sf(si2_a);
        var si2_b := SI(so2_a.outputs, si.state[|mf_a.state|..]);
        var so2_b := mf_b.sf(si2_b);
        var so2 := SO(so2_b.outputs, so2_a.state + so2_b.state);
        // Comparison
        assert si1_a == si2_a;
        assert si1_b == si2_b;
        assert so1_a == so2_a;
    ...
    lemma ConnectionKeysBInputs()
      requires Valid()
      ensures Conn().GetConnection().Keys == Seq.ToSet(mf_b.inputs)
    {
      var conn := Conn();
    ...
    ensures (Seq.ToSet(e1.mf.state) !! Seq.ToSet(e2.mf.state))
  {
    ...
    combiner.ConnectionKeysBInputs();
    assert conn.GetConnection().Keys !! c.PortSource.Keys by {
      assert IsIsland(c, e2.sc);
      InputsOfIslandNotInPortSource(c, e2);
    }
    (e12, conn)
  }
}