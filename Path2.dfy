module Path2 {
  ...
  lemma EvaluateReduceFI(c: Circuit, np: NP, fi: FI, removed: set<NP>)
    requires c.Valid()
    requires NPValid(c, np)
    requires FICircuitValid(c, fi)
    requires !PathExistsToNPSet(c, np, removed)
    ensures
      var reduced_fi := FI(fi.inputs - removed, fi.state);
      ...
      Evaluate(c, np, fi) == Evaluate(c, np, reduced_fi)
  {
    ...
    reveal FICircuitValid();
    if ONPValid(c, np) {
      EvaluateONPReduceFI(c, np, fi, removed);
    } else {
      EvaluateINPReduceFI(c, np, fi, removed);
    }
  }

  lemma EvaluateONPReduceFI(c: Circuit, np: NP, fi: FI, removed: set<NP>)
    requires c.Valid()
    requires FICircuitValid(c, fi)
      ...
      EvaluateONP(c, np, fi) == EvaluateONP(c, np, reduced_fi)
  {
    ...
    EvaluateONPInnerReduceFI(c, p, fi, removed);
  }

  lemma EvaluateINPReduceFI(c: Circuit, np: NP, fi: FI, removed: set<NP>)
    requires c.Valid()
    requires FICircuitValid(c, fi)
      ...
      EvaluateINP(c, np, fi) == EvaluateINP(c, np, reduced_fi)
  {
    ...
    LengthOneNoDuplicates(path);
    reveal PathValid();
    EvaluateINPInnerReduceFI(c, path, fi, removed);
  }

  lemma EvaluateONPInnerReduceFI(c: Circuit, p: seq<NP>, fi: FI, removed: set<NP>)
    requires c.Valid()
    requires PathValid(c, p)
    requires EvaluateONPInnerRequirements(c, p, fi)
    requires !PathExistsToNPSet(c, Seq.Last(p), removed)
    ensures
      var reduced_fi := FI(fi.inputs - removed, fi.state);
      reveal FICircuitValid();
    ...
    decreases |NodesNotInPath(c, p)|, 4
  {
    ...
    if head.n in fi.state {
      assert head.n in reduced_fi.state;
      assert nk.CSeq? by {
        reveal FICircuitValid();
      }
      match nk
      ...
      match nk
        case CXor() => EvaluateONPBinaryReduceFI(c, p, fi, removed);
        case CAnd() => EvaluateONPBinaryReduceFI(c, p, fi, removed);
        case CInv() => EvaluateONPUnaryReduceFI(c, p, fi, removed);
        case CIden() => EvaluateONPUnaryReduceFI(c, p, fi, removed);
        case CConst(b) => {
          reveal PathCaught();
        }
        case CSeq() => {
  ...
  lemma EvaluateONPBinaryReduceFI(c: Circuit, path: seq<NP>, fi: FI, removed: set<NP>)
    requires EvaluateONPBinaryRequirements(c, path, fi)
    requires !PathExistsToNPSet(c, Seq.Last(path), removed)
    ensures
      var reduced_fi := FI(fi.inputs - removed, fi.state);
      reveal FICircuitValid();
    ...
    decreases |NodesNotInPath(c, path)|, 3
  {
    ...
    if inp_0 in path {
      ...
      StillNoPathExistsToNPSet(c, Seq.Last(path), inp_0, removed);
      StillNoPathExistsToNPSet(c, Seq.Last(path), inp_1, removed);
      EvaluateINPInnerReduceFI(c, path + [inp_0], fi, removed);
      EvaluateINPInnerReduceFI(c, path + [inp_1], fi, removed);
    }
  }

  lemma EvaluateONPUnaryReduceFI(c: Circuit, path: seq<NP>, fi: FI, removed: set<NP>)
  ...
  lemma EvaluateINPInnerReduceFI(c: Circuit, path: seq<NP>, fi: FI, removed: set<NP>)
    ...
    decreases |NodesNotInPath(c, path)|, 2
  {
    ...
    if head in fi.inputs {
      assert head !in removed by {
        if head in removed {
          reveal PathExistsToNPSet();
          var p_contradict := [head];
          reveal PathValid();
          assert PathToNPSet(c, p_contradict, head, removed);
          assert false;
        }
      }
      ...
      if head in c.PortSource {
        ...
        if onp in path {
          ...
          StillNoPathExistsToNPSet(c, Seq.Last(path), onp, removed);
          EvaluateONPInnerReduceFI(c, path + [onp], fi, removed);
          reveal PathCaught();
        }
      } else {
      }
